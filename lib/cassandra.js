...
Cassandra.prototype.generateValueByColumnType = function(type) {
  var Uuid = this.dataSource.driver.types.Uuid;
  var val = null;
  type = (type.name || type);
...
function getUuid(cassandra) {
  var self = cassandra;
  var Uuid = self.dataSource.driver.types.Uuid;
  var val = Uuid.random();
  return val;  
}
/*!
...
Cassandra.prototype.toColumnValue = function(prop, val) {
  ...
  if (val == null) {
    if (prop.autoIncrement || prop.id) {
      // SETO return new ParameterizedSQL('DEFAULT');
      val = getUuid(this);
      return val;
    }
    return null;
