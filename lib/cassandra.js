...
function cassandraOptions(source) {
  var defaults = { prepare : true, readTimeout: 30000 };
  if (source){
    defaults = _.extend(defaults, source);
  }
  return defaults;
}
...
function defineCassandraTypes(dataSource) {
  ...
  // defineType(uuidFromString, ['Uuid']);
}
...
Cassandra.prototype.executeSQL = function(cql, params, options, callback) {
  ...
  this.client.execute(cql, params, { prepare : true, readTimeout: 30000 }, myCallback);
};
...
Cassandra.prototype._modifyOrCreate = function(model, data, options, fields, cb) {
  ...
  this.execute(sql.sql, sql.params, options, function(err, data) {
    if (!err && !data && fieldNames && columnValues &&
      fieldNames.length > 0 && fieldNames.length === columnValues.length) {
      data = {};
      fieldNames.forEach(function(fieldName, ix) {
        var value = columnValues[ix].params[0];
        if (columnValues[ix]) data[fieldName] = value;
      });
    }
    return cb(err, data);
  });
...
Cassandra.prototype.create = function(model, data, options, callback) {
  ...
  if (!data[idColName]) data[idColName] = stmt.params[stmt.params.length - 1];  // stmt.params[0]; // 
...
// function reorderKeys(keys, idColName) {
//   var ixId = keys.indexOf(idColName);
//   if (ixId >= 0) {
//     keys.splice(ixId, 1);
//     keys = [idColName].concat(keys);
//   }
//   return keys;
// }
