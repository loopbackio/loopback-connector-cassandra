...
function mixinMigration(Cassandra) {
  ...
  Cassandra.prototype.buildColumnDefinitions =
    Cassandra.prototype.propertiesSQL = function(model) {
      ...
      var cks = ss && Array.isArray(ss.clusteringKeys) ? ss.clusteringKeys : [];
  ...
  Cassandra.prototype.createTable = function(model, cb) {
    var self = this;
    var schemaName = self.escapeName(self.dataSource.settings.keyspace);
    var sqlSchema = 'CREATE SCHEMA IF NOT EXISTS ' + schemaName +
      ' WITH REPLICATION = { \'class\' : \'SimpleStrategy\',' +
      ' \'replication_factor\' : \'3\' }';
    self.execute(sqlSchema,
      function(err) {
        if (err) {
          return cb && cb(err);
        }
        self.execute('USE ' + schemaName, function(err) {
          if (err) {
            return cb && cb(err);
          }
          ...
          self.execute(sqlTable, cb);
          // TSETO: auto index creation prototype
          // self.execute(sqlTable, function(err) {
          //   if (err) {
          //     return cb && cb(err);
          //   }
          //   var idCol = self.idColumn(model);
          //   var cols = Object.keys(self.getModelDefinition(model).properties);
          //   cols = cols.filter(function(col) {return col !== idCol;});
          //   async.eachSeries(cols, function(col, asyncCb) {
          //     col = self.escapeName(col);
          //     var sqlIndex = 'CREATE INDEX ON ' + tableName + ' (' + col + ')';
          //     self.execute(sqlIndex, asyncCb);
          //   }, function(err) {
          //     if (cb) cb(err);
          //   });
          // });
