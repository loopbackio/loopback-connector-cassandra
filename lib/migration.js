    ...
    self.execute(sqlKeyspace, function(err) {
      ...
      self.execute('USE ' + ksName, function(err) {
        ...
        Object.keys(properties).forEach(function(prop) {
          ...
          if (properties[prop].customIndex) customIndexCols.push({"colName":colName,"data":properties[prop].customIndex});
        });
        ...
        self.execute(dropTable, function(err) {
          ...
          self.execute(sqlTable, function(err) {
            if (err) return cb && cb(err);
            async.eachSeries(indexCols.concat(customIndexCols), function(col, callback) {
              var sqlIndex="";
              var colName=col.colName?col.colName:col;
              if(col.data){
                sqlIndex = 'CREATE CUSTOM INDEX ON ' + tableName + ' (%s)';
                var data=col.data;
                if(data.using)sqlIndex = sqlIndex.concat(f(" USING '%s'",data.using));
                if(data.options)sqlIndex = sqlIndex.concat(f(' WITH OPTIONS = %s',JSON.stringify(data.options).replace(/"/g,"'")));
              }else{
                 sqlIndex = 'CREATE INDEX ON ' + tableName + ' (%s)';
              }
              var sql = f(sqlIndex, colName);
